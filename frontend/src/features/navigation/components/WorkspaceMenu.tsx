...
import type { AIUsage, Space, WorkspaceState } from "../../../shared/model/types"
		...
		<div ref={wrap} className="relative min-w-0 flex-1">
			...
			{open && box
				...
				<div
					...
					className="bg-sidebar border-sidebar-border shadow-sheet animate-in fade-in-0 zoom-in-95 slide-in-from-top-1 fixed z-[80] flex origin-top flex-col overflow-hidden rounded-xl border duration-150"
				>
							...
							<div key={account.userId} className="pb-1">
								...
								{account.spaces.map((space) => (
									<button
										...
										className={cn(
											"hover:bg-sidebar-accent focus-visible:ring-ring flex h-9 w-full cursor-pointer items-center gap-2 rounded-md px-2 text-left transition-colors focus-visible:ring-2",
											busy && "opacity-60",
										)}
									>
