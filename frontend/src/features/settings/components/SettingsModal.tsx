...
/**
 * Локальные (stdio) MCP-серверы: пакеты, которые запускаются через npx
 * на этой машине. Notion подключает только сетевые адреса, поэтому notcode
 * поднимает пакет дочерним процессом и отдаёт его через тот же туннель.
 * needsEnv — без этой переменной сервер не стартует, значит сначала диалог.
 */
const STDIO_PRESETS: Array<{ name: string; title: string; command: string; args: string[]; hint: string; needsEnv?: string; needsArgs?: boolean }> = [
	{ name: "playwright-local", title: "Playwright (локально)", command: "npx", args: ["-y", "@playwright/mcp@latest"], hint: "Браузер на этом компьютере, без Smithery и ключей." },
	{ name: "memory-local", title: "Memory (граф знаний)", command: "npx", args: ["-y", "@modelcontextprotocol/server-memory"], hint: "Память агента в локальном файле." },
	{ name: "filesystem-local", title: "Filesystem", command: "npx", args: ["-y", "@modelcontextprotocol/server-filesystem"], hint: "Доступ к папкам: добавьте пути в аргументы.", needsArgs: true },
	{ name: "context7-local", title: "Context7 (локально)", command: "npx", args: ["-y", "@upstash/context7-mcp"], hint: "Документация библиотек. Ключ опционален." },
	{ name: "github-local", title: "GitHub (локально)", command: "npx", args: ["-y", "@modelcontextprotocol/server-github"], hint: "Нужен GITHUB_PERSONAL_ACCESS_TOKEN.", needsEnv: "GITHUB_PERSONAL_ACCESS_TOKEN" },
	{ name: "sequential-thinking", title: "Sequential Thinking", command: "npx", args: ["-y", "@modelcontextprotocol/server-sequential-thinking"], hint: "Пошаговые рассуждения для сложных задач." },
]
...
// Аргументы из одной строки: пути в Windows часто с пробелами, поэтому уважаем кавычки.
...
export default function SettingsModal({ open, onOpenChange, navWidth = 276, settings, models, connection, initialSection, onSaved, onConnectionChange, onToast }: {
	...
	const [stdioDialog, setStdioDialog] = useState(false)
	const [newStdio, setNewStdio] = useState({ name: "", command: "npx", args: "", env: "", autoRun: true })
	...
	return <div className="bg-background absolute inset-0 z-40 flex min-h-0 min-w-0">
		...
		<Dialog open={stdioDialog} onOpenChange={setStdioDialog}><DialogContent className="max-w-lg p-0"><div className="border-b p-5"><DialogTitle>Локальный MCP-сервер (stdio)</DialogTitle><DialogDescription>Пакет запускается на этом компьютере, а Notion подключается к нему через туннель notcode.</DialogDescription></div><div className="space-y-4 p-5"><div><Label>Название</Label><Input className="mt-2" value={newStdio.name} onChange={e => setNewStdio(current => ({ ...current, name: e.target.value }))} placeholder="Playwright" /></div><div><Label>Команда</Label><Input className="mt-2" value={newStdio.command} onChange={e => setNewStdio(current => ({ ...current, command: e.target.value }))} placeholder="npx" /></div><div><Label>Аргументы</Label><Input className="mt-2" value={newStdio.args} onChange={e => setNewStdio(current => ({ ...current, args: e.target.value }))} placeholder="-y @playwright/mcp@latest" /><p className="text-muted-foreground mt-1 text-xs">Через пробел; аргументы с пробелами берите в кавычки.</p></div><div><Label>Переменные окружения</Label><Input className="mt-2" value={newStdio.env} onChange={e => setNewStdio(current => ({ ...current, env: e.target.value }))} placeholder="GITHUB_PERSONAL_ACCESS_TOKEN=ghp_..." /></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={newStdio.autoRun} onChange={e => setNewStdio(current => ({ ...current, autoRun: e.target.checked }))} />Разрешить запуск инструментов автоматически</label><div className="flex justify-end gap-2"><Button variant="secondary" onClick={() => setStdioDialog(false)}>Отмена</Button><Button variant="brand" disabled={!newStdio.name.trim() || !newStdio.command.trim()} onClick={() => run("add-stdio", async () => { await api.mcpInstallStdio({ name: newStdio.name.trim(), command: newStdio.command.trim(), args: parseArgs(newStdio.args), env: parsePairs(newStdio.env, "="), autoRun: newStdio.autoRun, runWriteToolsAutomatically: newStdio.autoRun }); setModules(await api.mcpList()); setNewStdio({ name: "", command: "npx", args: "", env: "", autoRun: true }); setStdioDialog(false); onToast("Локальный MCP-сервер подключён") })}>Запустить и подключить</Button></div></div></DialogContent></Dialog>
