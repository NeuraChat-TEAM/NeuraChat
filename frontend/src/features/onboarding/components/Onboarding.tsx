...
import { api, errText } from "../../../shared/api/api"
import type { Account, ConnectionState } from "../../../shared/model/types"
...
import type { Artifact } from "../../artifacts/model/artifacts"
...
export default function Onboarding({
	...
	async function refresh() {
		try {
			const [ws, ngrok] = await Promise.all([api.listWorkspaces(), api.ngrokTokenStatus()])
			setAccounts(ws?.accounts ?? [])
			setNgrokConfigured(ngrok.configured)
		} catch {
	...
	const ready = accounts.length > 0 && ngrokConfigured
	...
	return (
		<ScrollArea className="min-h-0 flex-1">
			<div className="mx-auto flex w-full max-w-[680px] flex-col gap-4 px-6 pt-16 pb-12">
				...
				{step > GREETING.length ? (
					<div className="answer-reveal bg-card mt-2 flex flex-col gap-3 rounded-2xl border p-4 shadow-xs">
						...
						<textarea
							...
							spellCheck={false}
							placeholder="Вставьте Copy as cURL из DevTools"
				...
				{accounts.length > 0 && !ngrokConfigured ? (
					<div className="answer-reveal bg-card flex flex-col gap-3 rounded-2xl border p-4 shadow-xs">
						<div className="flex items-center gap-1.5"><span className="text-[13px] font-medium">Теперь подключите ngrok</span><Popover><PopoverTrigger asChild><button type="button" aria-label="Как получить ngrok authtoken" className="text-muted-foreground hover:text-foreground grid size-5 place-items-center rounded-full"><HelpCircle className="size-4" /></button></PopoverTrigger><PopoverContent className="w-[340px]"><p className="mb-2 text-[13px] font-medium">Как получить authtoken</p><ol className="text-muted-foreground list-decimal space-y-1 ps-4 text-xs leading-5"><li>Откройте dashboard.ngrok.com и войдите в аккаунт.</li><li>Перейдите в Getting Started → Your Authtoken.</li><li>Скопируйте только значение токена и вставьте сюда.</li></ol></PopoverContent></Popover></div>
						<p className="text-muted-foreground text-xs">Он нужен встроенным инструментам, чтобы Notion мог подключиться к локальному NotCode.</p>
						<input type="password" autoComplete="off" value={ngrokToken} onChange={(e) => setNgrokToken(e.target.value)} placeholder="ngrok authtoken" className="bg-background h-9 rounded-lg border px-3 text-sm outline-none focus-visible:border-[var(--blue-accent)]" />
						<div className="flex justify-end"><Button variant="brand" disabled={!ngrokToken.trim() || savingNgrok} onClick={() => { setSavingNgrok(true); void api.saveNgrokToken(ngrokToken.trim()).then(() => { setNgrokConfigured(true); setNgrokToken(""); onToast("ngrok подключён") }).catch((e) => onToast(errText(e), true)).finally(() => setSavingNgrok(false)) }}>{savingNgrok ? <Loader2 className="size-3.5 animate-spin" /> : <Check className="size-3.5" />}Сохранить и проверить</Button></div>
					</div>
				) : null}
				...
				{ready ? (
					<div className="answer-reveal flex flex-col gap-3">
						<p className="text-muted-foreground text-[13px]">
							Готово — Notion и встроенные инструменты настроены:
						</p>
