...
import type { Thread, WorkspaceState } from "../lib/types"
...
function clampTitle(raw: string, width: number) {
	...
	const maxChars = Math.max(10, Math.floor((width - 104) / 7.2))
	...
	return (
		<div ref={wrap} className="relative shrink-0">
			<button
				...
				className={cn(
					"text-muted-foreground hover:bg-accent hover:text-foreground size-6 place-items-center rounded transition-colors",
					open ? "bg-accent text-foreground grid" : "hidden group-hover:grid",
				)}
			>
			...
			{open ? (
				...
				>
					...
					<p className="text-muted-foreground px-2 pt-1.5 pb-1 text-[11px]">
						Изменён {relTime(thread.updatedAt)} назад
					</p>
				</div>
			) : null}
		</div>
		...
		return (
			<li key={t.id} className="min-w-0">
				<div
					...
					className={cn(
						"group flex h-[30px] min-w-0 cursor-pointer items-center gap-2 rounded-md px-1.5 transition-colors",
