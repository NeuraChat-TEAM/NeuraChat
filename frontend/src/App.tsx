...
import { api, errText, onChatEvent, type UploadedAttachment } from "./lib/api"
import type {
	ChatEvent,
	ConnectionState,
	Model,
...
import Sidebar, { SIDEBAR_DEFAULT, SIDEBAR_MAX, SIDEBAR_MIN } from "./components/Sidebar"
...
export default function App() {
	...
	// API присылает дельты огромными кусками; показываем их потоком
	// на каждом кадре, как в веб-версии Notion AI.
	const textQueue = useRef("")
	const revealRaf = useRef(0)
	const revealLast = useRef(0)
	...
	const pumpReveal = useCallback(() => {
		...
		const dt = Math.min(64, Math.max(8, now - (revealLast.current || now)))
		revealLast.current = now
		const left = textQueue.current
		if (!left) return
		// Ровный поток: очередь рассасывается примерно за 450 мс,
		// но не медленнее ~140 и не быстрее ~1100 символов в секунду.
		// Так текст «печатается» плавно, без рывков и вставок кусками.
		const cps = Math.min(1100, Math.max(140, left.length / 0.45))
		const take = Math.max(1, Math.round((dt / 1000) * cps))
		appendText(left.slice(0, take))
		textQueue.current = left.slice(take)
		if (textQueue.current) revealRaf.current = requestAnimationFrame(pumpReveal)
	}, [appendText])
	...
	const enqueueText = useCallback(
		(delta: string) => {
			...
			if (!revealRaf.current) {
				revealLast.current = performance.now()
				revealRaf.current = requestAnimationFrame(pumpReveal)
			}
		},
		[pumpReveal],
	)

	// Перед любым другим событием досыпаем остаток — иначе порядок
	// текста и карточек шагов сломается.
	...
	const applyEvent = useCallback(
		(ev: ChatEvent) => {
			if (ev.type === "thread-title" && ev.title) {
				setThreads((prev) =>
					prev.map((t) =>
						t.id === ev.threadId || t.id === threadId ? { ...t, title: ev.title! } : t,
					),
				)
				return
			}
		...
		[threadId, notify, enqueueText, flushText],
	)
	...
	async function send(files: Attachment[] = [], override?: string) {
		...
		try {
			...
			setTurns((prev) => {
				...
				const done = next.find((t) => t.id === assistantId.current)
				if (done && tid) void persistTurn(tid, done)
				return next
			})
			api.listThreads()
				.then((l) => setThreads(l ?? []))
	...
	async function openFile(file: ComputerFile) {
		try {
			...
			if (got.signedUrl) {
				await api.openURL(got.signedUrl)
				return
			}
